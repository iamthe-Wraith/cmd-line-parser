...
class CmdLineParser {
      ...
      this.pattern = parameters.map(parameter => {
        ...
        if (parsedParam.indexOf('?') > -1) {
          ...
          if (optionalParamFound) {
            throw new Error('required parameter cannot be after optional parameter');
          }
    ...
    [longHand, shortHand].filter(Boolean).forEach(id => {
      ['argument', 'flag'].forEach(type => {
        ...
        if (registry.hasOwnProperty(id)) {
          throw new Error(`${type} with identifier ${id} already exists`);
        }
      });
    });
  }
  ...
  argument (name, opts = {}) {
    if (!name) {
      throw new Error('no name found in argument registration');
    }
  ...
  parameter (name, opts = {}) {
    if (!name) {
      throw new Error('no name found in parameter registration');
    }
    ...
    if (paramFoundInPattern) {
      if (!this.parameters.hasOwnProperty(name)) {
        ...
        throw new Error(`${name} is already a registered parameter`);
      }
    } else {
      throw new Error(`${name} was not specified in pattern`);
    }
  ...
  flag (name, opts = {}) {
    if (!name) {
      throw new Error('no name found in flag registration');
    }
      ...
      [`--${arg}`, `-${shortHand}`].filter(Boolean).forEach(i => {
        ...
        if (index !== -1) {
          ...
          if (args[index + 1] !== undefined) {
            ...
            throw new Error(`no value passed to ${i}`);
          }
          ...
          if (typeof this.arguments[arg].validate === 'function') {
            if (this.arguments[arg].validate(castedValue)) {
              ...
              throw new Error(`${arg} failed validation`);
            }
          } else {
            parsed[arg.split('-').join('')] = castedValue;
          }
  ...
  parseParameters (args) {
    ...
    if (args.length > this.pattern.length) {
      throw new Error(`invalid command structure - expected ${this.pattern.length} parameters, but found ${args.length}`);
    } else {
      for (let i = 0; i < this.pattern.length; i++) {
        if (args[i] !== undefined) {
          ...
          if (this.pattern[i].name in this.parameters) {
            ...
            throw new Error(`${this.pattern[i].name} is not a registered parameters`);
          }
          ...
          if (this.pattern[i].required) {
            throw new Error(`${this.pattern[i].name} is required`);
          }
        }
      }
    }
   ...
   *
   * @return {Promise} - updated context with parsed arguments and resolves with context
   ***/
  parse (ctx) {
    return new Promise((resolve, reject) => {
      try {
        if ('args' in ctx) {
          ...
          reject(new Error('no args found in context - Parser:parse'));
        }
      } catch (err) {
        reject(err);
      }
